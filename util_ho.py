...
class LbToLclNu_Model:
    ...
    def get_ff_params(self):
        ...
        for FF in (set(self.FFs) - set(self.non_indp_FFs)): 
    ...
    def pdf_integral(self, norm_sample, method = '1', ranges = None):
        ...
        if ranges is None:
            ranges = self.phase_space.ranges
    ...
    def plot_fitresult_binned_2D(self, np_data, np_fit, np_data_err = None, bin_scheme = 'Scheme0', fname = './fitres.pdf', xlabel = "q^{2} [GeV^{2}]", ylabel = "cos(#theta_{#mu})"):
        ...
        for i in range(tot_bins):
            ...
            if np_data_err is not None:
                pull_dn = np.sqrt(np_data[i])
                #pull_dn = np.sqrt(np_data[i] + np_fit[i])
            else: 
                np_data_err =  np_data_err.flatten()
                pull_dn     = np_data_err[i]
    ...
    def plot_fitresult_binned_1D(self, var, np_data_x, np_fitres_x, bin_scheme, fname, label):
        ...
        ymin = max(min(cmin), 1e-10); ymax = max(cmax)+5e4
