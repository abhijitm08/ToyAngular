...
def main():
    ...
    if use_data: 
        print('Using data')
        b_data = pickle.load( open( home+"/LbToLclnu_RunTwo/FittingScripts/mcorr_spectrum/mcorr_data.p", "rb" ) )
        b_data_err = pickle.load( open( home+"/LbToLclnu_RunTwo/FittingScripts/mcorr_spectrum/mcorr_data_err.p", "rb" ) )
    else:
        if unbinned_toygen:
            print('Generating unbinned toy data')
            ...
            print('Generating binned toy data')
    ...
    print('Binned data shape is', b_data.shape)
    ...
    if plotRes: 
        model.plot_fitresult_binned_2D(b_data, b_fit, bin_scheme = bscheme, fname = plotfname, np_data_err = b_data_err)
        b_data_x = np.einsum('ij->i', b_data).flatten()
        b_fit_x  = np.einsum('ij->i', b_fit).flatten()
        b_data_y = np.einsum('ij->j', b_data).flatten()
        b_fit_y  = np.einsum('ij->j', b_fit).flatten()
        model.plot_fitresult_binned_1D('qsq' , b_data_x, b_fit_x, bscheme, plotfname.replace('.pdf', '_x.pdf'), "q^{2} [GeV^{2}]")
        model.plot_fitresult_binned_1D('cthl', b_data_y, b_fit_y, bscheme, plotfname.replace('.pdf', '_y.pdf'), "cos(#theta_{#mu})")
...
if __name__ == '__main__':
    ...
    parser.add_argument('-ud', '--use_data', dest='use_data' ,type=str2bool,default='False',help='(bool) Set to True if you want to use data to fit the toys. Default is False.')
